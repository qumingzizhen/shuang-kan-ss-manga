...
5. 响应类型、体积上限、最小有效图片与 JPEG/PNG/GIF/WebP/AVIF 文件签名校验。
6. 临时文件原子写入与失败清理。
7. 小于最小有效体积或签名无效的旧缓存自动删除并重新拉取。

缓存目录只在整条重试链开始时创建一次；相同 URL 的检查、修复和重试均在 single-flight 中完成。缓存命中仅读取前 32 字节识别签名，路由直接复用缓存层返回的文件大小与 MIME，避免重复 stat 和按扩展名误判。
...
模块测试模拟两个并发浏览器请求、第一次上游返回 503、第二次成功，证明重试链受 single-flight 合并；随后主动写入错误签名缓存，下一次读取会自动淘汰并只增加一次上游请求。前端组件测试逐次触发三次图片错误，验证前两次进入等待状态，达到上限后才进入最终失败态。
