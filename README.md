...
5. 按“来源 + 漫画链接”去重；
6. 返回合并结果及单个来源的错误信息；
7. 在自适应封面画廊接近底部时，按页继续请求各来源并追加去重后的结果。

搜索任务默认每批最多获取 40 条候选结果。任务页使用可折叠的新建任务区和全宽紧凑列表，每个任务只保留标题、状态、进度、结果摘要、更新时间与操作，不再在列表行内嵌套封面画廊；完整搜索结果从任务详情进入宽屏画廊查看。画廊会根据窗口宽度自动调整为 5/4/3/2 列，初次只渲染 10 项，继续下滑会先分批显示已获取结果，再通过 `POST /v1/tasks/{id}/search-more` 请求下一搜索页，避免一次渲染大量卡片阻塞界面。
