...
test("搜索结果使用封面网格、按最新上传排列，并把操作与 Tag 放在二级详情", async ({ page }) => {
  await page.route("**/v1/sources", (route) => route.fulfill({
    json: [{
      ...
      name: "Fangliding",
      version: "1.0.0",
      capabilities: ["search", "gallery", "download", "page_list", "page_image", "online_read"],
      enabled: true,
    }],
  }));
  await page.route("**/v1/tasks", (route) => route.fulfill({
    json: [{
      ...
      status: "completed",
      title: "排序界面测试",
      payload: { source_ids: ["fangliding"], tags: [] },
        ...
        results: [
          {
            ...
            gallery_url: "https://ex.fangliding.eu.org/g/1/old/",
            title: "较早作品",
          ...
          {
            source_id: "fangliding",
            gallery_url: "https://ex.fangliding.eu.org/g/2/new/",
            title: "最新作品",
            tags: [],
            uploader: "new-user",
  ...
  await expect(page.getByRole("button", { name: "在线阅读", exact: true })).toBeEnabled();
