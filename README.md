...
搜索任务默认每批最多获取 40 条候选结果。任务表只显示紧凑封面预览，完整结果通过宽屏画廊查看；画廊会根据窗口宽度自动调整为 5/4/3/2 列。初次只渲染 10 项，继续下滑会先分批显示已获取结果，再通过 `POST /v1/tasks/{id}/search-more` 请求下一搜索页，避免一次渲染大量卡片阻塞界面。
