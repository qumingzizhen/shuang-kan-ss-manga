...
搜索结果二级详情中的“在线阅读”曾因 Fangliding 来源仅声明搜索、图库和下载能力而被前后端正确地禁用；问题不是按钮样式，而是该桥接器尚未实现标准页命令。

本次补齐 `list-pages` 与 `download-page`：前者复用现有图库元数据采集并输出统一页描述，后者复用现有请求客户端、挑战页检测、图片 URL 解析、二进制重试和扩展名识别，再以临时文件替换方式写入阅读缓存。来源配置同步声明 `page_list`、`page_image`、`online_read`，没有在界面中按来源硬编码放行。

回归验证包括：

- 离线契约测试验证页列表、索引、单页缓存文件、内容类型、字节数和临时文件清理；
- 桌面端与移动端浏览器测试使用 Fangliding 来源，并明确断言在线阅读按钮处于启用状态；
- 真实公开页面完成搜索、65 页阅读会话创建及首张 WebP 经本地 API 返回的完整链路验证；测试会话和缓存随后清理。
